...
/**
 * Bulk soft-delete from the dashboard list. Mirrors the frontend BulkActionBar's
 * two selection modes:
 *  - explicit: { ids: [...] }
 *  - all-matching: the same filter params the list uses, plus { exclude: [...] }
 *    for rows the user un-ticked.
 *
 * Filter parsing is shared with the list (ParsesApplicationFilters) so the
 * deleted set is exactly what the user saw. A request with neither ids nor any
 * active filter is rejected — bulk delete is always scoped, never "everything".
 */
class DeleteRequest extends FormRequest
{
		...
		$validator->after(function (Validator $validator) {
			if (! $this->hasExplicitIds() && $this->search() === null && $this->filters() === []) {
				$validator->errors()->add(
					'ids',
					'Bulk-Löschung benötigt eine Auswahl (ids) oder einen aktiven Filter.'
				);
			}
		});
	}
	...
	/**
	 * @return array<int, int>  Ids to leave out of an all-matching delete.
	 */
